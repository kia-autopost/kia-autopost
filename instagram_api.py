"""
Instagram API — uploads reel to Cloudinary, then posts via Instagram Graph API.
"""
...
def _upload_to_cloudinary(video_path, cloud, key, secret):
        ...
        result = cloudinary.uploader.upload_large(video_path, resource_type='video', folder='kia_reels', overwrite=True)
...
def _create_container(ig_user_id, access_token, video_url, caption):
        ...
        resp = requests.post(f'{GRAPH}/{ig_user_id}/media', params={'media_type':'REELS','video_url':video_url,'caption':caption,'share_to_feed':'true','access_token':access_token}, timeout=30)
        resp.raise_for_status()
        data = resp.json()
        if 'error' in data: raise RuntimeError(f"Instagram container error: {data['error']}")
...
def _wait_for_ready(container_id, access_token, max_wait=300):
    ...
    while time.time() - start < max_wait:
                resp = requests.get(f'{GRAPH}/{container_id}', params={'fields':'status_code','access_token':access_token}, timeout=15)
                resp.raise_for_status()
                status = resp.json().get('status_code', '')
                log.info(f"  Status: {status}")
                if status == 'FINISHED': return
                            if status == 'ERROR': raise RuntimeError("Instagram video processing failed")
                                        time.sleep(10)
...
def _publish(ig_user_id, access_token, container_id):
    ...
    resp = requests.post(f'{GRAPH}/{ig_user_id}/media_publish', params={'creation_id':container_id,'access_token':access_token}, timeout=30)
    resp.raise_for_status()
    data = resp.json()
    if 'error' in data: raise RuntimeError(f"Instagram publish error: {data['error']}")
            media_id = data['id']
...
def _refresh_token(access_token):
        """
            Refresh a long-lived Graph API user token (valid 60 days, refreshable).
